...
#'  the clustering was based. When the GAP criterion was used in \code{Step3Clusters}
#'  to determine the optimal number of clusters, a plot of the GAP statistic as
#'  a function of the number of clusters is provided.
#'
#'@param trajClusters object of class \code{trajClusters} as returned by
#'  \code{Step3Cluster}.
...
#'
#'@export
plot.trajClusters <- function(trajClusters, sample.size = 5, ask = TRUE, which.plots = NULL, spline = FALSE){
  
  if(!is.null(which.plots) & ( !is.numeric(which.plots) | !is.vector(which.plots) )){
    stop("The argument 'which.plots' should be a subset of the plots required, specified as a vector such as c(1, 2, 5:7), or NULL if all the plots are required.")
  }
  ...
  plot.counter <- 0 # This labels the plots that appear when 'which.plots' is set to NULL 
  
  ### GAP statistic ###
  
  gap.condition <- ((is.null(which.plots) | (1 %in% which.plots)) & !is.null(trajClusters$GAP))
  if (gap.condition) {
  ...
  is.different <- c()
  for (i in 1:nrow(trajClusters$time)) {
    is.different[i] <- !identical(trajClusters$time[i, -1], universal.time)
  }
  homogeneous.times <- (sum(is.different) == 0)
  
  med.mean.condition <- ( (spline | homogeneous.times) & ( is.null(which.plots) | sum( c((plot.counter + 1):(plot.counter + 4)) %in% which.plots) > 0) ) 
  if (med.mean.condition) {
    
    if (is.null(which.plots)) {
      which.med.mean <- c(1:4)
    } else{
      which.med.mean <- which(c((plot.counter + 1):(plot.counter + 4)) %in% which.plots)
    }
    ...
    if (spline == TRUE) {
      ...
      for (i in 1:nrow(time)) {
        time.new[i, ] <- seq(from = a, to = b, length.out = N)
        
        for(j in 1:N){
          if( time.new[i, j] < min(time[i, ], na.rm=TRUE) | time.new[i, j] > max(time[i, ], na.rm=TRUE)){
            time.new[i, j] <- NA
          }
        }
        spl.i <- smooth.spline(x = na.omit(time[i, ]), y = na.omit(data[i, ]), cv = FALSE)
    ...
    for (j in 1:trajClusters$nclusters) {
      ...
      col.keep <- colSums(is.na(data.cluster.j)) < nrow(data.cluster.j) #identifies the columns which are not mono NA
      ...
      if (spline == TRUE) {
        time.for.plots[[j]] <- seq(from = a, to = b, length.out = N)[col.keep]
      } else {
      ...
      CI.low[[j]] <- mean.traj.j - qt(p = 0.025, df = df, lower.tail = FALSE) * sd.traj.j/sqrt(length(which.j))
      
      CI.high[[j]] <- mean.traj.j + qt(p = 0.025, df = df, lower.tail = FALSE) * sd.traj.j/sqrt(length(which.j))
      
    }
    
    if(spline == TRUE){
      universal.time <- seq(from = a, to = b, length.out = N)
      main.med <- paste("Pointwise median \n of smoothed trajectories", sep = "")
      main.mean <- paste("Pointwise mean \n of smoothed trajectories", sep = "")
      main.mean2 <- paste("Pointwise mean \n of smoothed trajectory \n and 95% CI" , sep="")
    } else {
      main.med <- paste("Pointwise median trajectories", sep = "")
      main.mean <- paste("Pointwise mean trajectories", sep = "")
      main.mean2 <- paste("Pointwise mean trajectory \n and 95% CI" , sep="")
    }
    ...
    if (1 %in% which.med.mean) {
      ...
      plot(x = 0, y = 0, xlim = c(min(universal.time), max(universal.time)), ylim = c(min(unlist(median.traj)), max(unlist(median.traj))), type = "n", xlab = "", ylab = "", main = main.med)
      legend("topright",col=color.pal[1:trajClusters$nclusters], legend=paste(1:trajClusters$nclusters), lty=rep(1,trajClusters$nclusters))
      
      for (j in 1:trajClusters$nclusters) {
        lines(x = time.for.plots[[j]], y = median.traj[[j]], type = "l", col = color.pal[j])
      } 
    }
    ...
    if (2 %in% which.med.mean) {
      ...
      for (j in 1:trajClusters$nclusters) {
        ...
        plot(x = time.for.plots[[j]], y = median.traj[[j]], type = "l", xlab = paste("Cluster ", j, " (n = ", trajClusters$partition.summary[j], ")", sep=""), ylab = "", main = main.med, col = color.pal[j])
      }
    }
    ...
    if (3 %in% which.med.mean) {
      ...
      plot(x = 0, y = 0, xlim = c(min(universal.time), max(universal.time)), ylim = c(min(unlist(mean.traj)), max(unlist(mean.traj))), type = "n", xlab = "", ylab = "", main = main.mean)
      legend("topright",col=color.pal[1:trajClusters$nclusters], legend=paste(1:trajClusters$nclusters), lty=rep(1,trajClusters$nclusters))
      
      
      for(j in 1:trajClusters$nclusters){
        lines(x = time.for.plots[[j]], y = mean.traj[[j]], type = "l", col = color.pal[j])
      }
    }
    ...
    if (4 %in% which.med.mean) {
      ...
      for (j in 1:trajClusters$nclusters) {
        plot(x = 0, y = 0, xlim = c(min(time.for.plots[[j]], na.rm = TRUE), max(time.for.plots[[j]], na.rm = TRUE)), ylim = c(min(CI.low[[j]], na.rm = TRUE), max(CI.high[[j]], na.rm = TRUE)), type = "n", xlab = paste("Cluster ", j, " (n = ", trajClusters$partition.summary[j], ")", sep=""), ylab = "", main = main.mean2)
        
        w <- which(is.na(CI.high[[j]]))
        if(length(w) > 0){
          polygon(c(rev(time.for.plots[[j]][-w]), time.for.plots[[j]][-w]), c(rev(CI.high[[j]][-w]), CI.low[[j]][-w]), col = 'grey80', border = NA)
        } else{
          polygon(c(rev(time.for.plots[[j]]), time.for.plots[[j]]), c(rev(CI.high[[j]]), CI.low[[j]]), col = 'grey80', border = NA)
        }
        
        lines(x = time.for.plots[[j]], y = mean.traj[[j]], col = color.pal[j], type = "l")
      }
    }
  }
  
  if (!(spline | homogeneous.times)) {
    print(paste("No pointwise median and pointwise mean trajectories were generated because the observation times are not the same for each trajectories. Set the 'spline' argument to TRUE to plot median and mean trajectories based on smoothing splines."))
  }
  ...
  random.condition <- ( is.null(which.plots) | (plot.counter + 1) %in% which.plots )
  
  if (random.condition) {
    
    traj.by.clusters <- list()
    for(k in 1:trajClusters$nclusters){
      traj.by.clusters[[k]] <- trajClusters$data[which(trajClusters$partition[, 2] == k), -c(1)]
    }
    
    time.by.clusters <- list()
    for(k in 1:trajClusters$nclusters){
      time.by.clusters[[k]] <- trajClusters$time[which(trajClusters$partition[, 2] == k), -c(1)]
    }
    ...
    for(k in 1:trajClusters$nclusters){
      for(i in 1:min(size,trajClusters$partition.summary[k])){
        lines(x = na.omit(smpl.time.by.clusters[[k]][i, ]), y = na.omit(smpl.traj.by.clusters[[k]][i, ]), type = "l", col = color.pal[k])
      }
      legend("topright",col=color.pal[1:k], legend=paste(1:3)[1:k], lty=rep(1,k))
    }
  }
  ...
  scatter.condition <- (nb.measures > 1) & ( is.null(which.plots) | sum( c((plot.counter + 1):(plot.counter + nb.measures)) %in% which.plots) > 0 )
  ...
  if (scatter.condition) {
    ...
    if(is.null(which.plots)){
      ...
      which.scatter <- which(c((plot.counter + 1):(plot.counter + nb.measures)) %in% which.plots)
    }
    ...
    for(k in 1:trajClusters$nclusters){
      selection.by.clusters[[k]] <- selection[which(trajClusters$partition[, 2] == k), ]
    }
    ...
    for (m in which.scatter) {
      ...
      for (n in c(1:(nb.measures-1))) {
        ...
        plot(x = 0, y = 0, xlim = c(min(selection[, m]), max(selection[, m])), ylim = c(min(selection[, -c(m)][, n]), max(selection[, -c(m)][, n])), type = "n", xlab = paste(colnames(selection[m])), ylab = paste(colnames(selection[, -c(m)])[n]), main = paste("Scatter plot of ", paste(colnames(selection[m])), " vs ", paste(colnames(selection[, -c(m)])[n]), sep = ""))
        
        for(k in 1:trajClusters$nclusters){
          
          lines(x = selection.by.clusters[[k]][, m], y = selection.by.clusters[[k]][, -c(m)][, n], type = "p", pch = 20, col = color.pal[k], bg = color.pal[k])
          
          legend("topright", lty = rep(0, k), pch = rep(16, k), col = color.pal[1:k], legend = paste(1:3)[1:k])
        }
      }
    }
  }
