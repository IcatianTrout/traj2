...
Step1Measures <- function (Data, Time = NULL, ID = FALSE, measures = 1:23, midpoint = NULL) {
  ...
  if (ID == TRUE) {
    ...
    IDvector <- c(1:nrow(data))
  } 
  
  if (identical(dim(time), dim(data))) { 
    if (identical(is.na(data), is.na(time)) == FALSE) {
      stop("Data has cells with no corresponding time or Time has cells with no corresponding data.")
    }
    ...
    if (!identical(order(time), c(1:length(time)))) {
  ...
  if(TRUE %in% (c(25:27) %in% measures)){
    ...
    if (is.null(midpoint)) {
      ...
      for (i in 1:nrow(data)) {
        ...
        mid.position[i] <- w
        if (w == length(v) | w == 1) { 
          flag <- c(flag, i)  #  the mid point can't be the first or the last point, so if this occurs, the ith row gets "flagged" 
        }  
      }
      if (length(flag) > 0) {
        ...
        if (length(Lines) == 1) { 
          warning(paste("When left blank, the 'midpoint' argument defaults to the observation time closests to the median time 0.5*(max(Time)-min(Time)), but this can't be either the first or last observation time. As a result, row ", Lines," has been removed. To avoid this, consider excluding measures 24-26 from the analysis or providing custom 'midpoint' values.", sep = ""))
        }
        if (length(Lines) >1) { 
          warning(paste("When left blank, the 'midpoint' argument defaults to the observation time closests to the median time 0.5*(max(Time)-min(Time)), but this can't be either the first or last observation time. As a result, rows ", noquote(paste(Lines, collapse=", ")), " have been removed. To avoid this, consider excluding measures 24-26 from the analysis or providing custom 'midpoint' values.", sep = ""))
        }
  ...
  for (j in 2:ncol(output)) {
    ...
    k_Cheb <- sqrt(100/0.3) #  The classical Chebychev bound. Approximately 18.26.
