check.correlation <- function(output, verbose = TRUE, is.return = FALSE, tresh=0.98){
  ...
  for(i_row in mes.names[-length(mes.names)]){
    ...
    for(i_col in res.names){
      ...
      if(abs(cor.mat[i_row, i_col]) > tresh){ #there used to be no "abs" here, a mistake imo.
        
        corr.var = rbind(corr.var, c(i_col, i_row))
        
        if(verbose){
          print(paste("Variables ",i_row, " and ", i_col, " are perfectly or almost perfectly correlated.", sep = ""))
          is.corr = TRUE
        }
      }
    }
